...
- 检索范围：近 3-7 个月内围绕 irregular sampled / asynchronous / irregular ICU time series classification 的顶会或领域强会论文，重点核对 NeurIPS 2025、AAAI 2026、ICLR 2026、ICML 2026、ICASSP 2026 官方页、OpenReview、论文页与 arXiv。
- 已排除全部黑名单论文；同时排除 ICLR workshop 条目、仅显示为 ICLR 投稿未确认接收的 PULSE、未能确认顶会接收的 MILM、偏 forecasting 的 MIRA/MM-ISTS/TFMixer、偏规则 MedTS 的 MedMamba/MedSpaformer/Time-CoT，以及 KDD 2025/ICML 2025 等时间窗口偏旧条目。本次仅保留全新工作 1 篇。

## 12. Rethinking Large Language Models for Irregular Time Series Classification in Critical Care

- 会议：ICASSP 2026
- 作者：Feixiang Zheng, Yu Wu, Cecilia Mascolo, Ting Dang
- 链接：https://arxiv.org/abs/2601.16516
- 机构仓储页：https://www.repository.cam.ac.uk/items/86268aaf-8188-4d63-a2d5-7b8f67746841
- 关键词：irregular ICU time series classification, LLM-based time series modeling, encoder design, multimodal alignment, critical care mortality prediction

### 场景、任务与核心难点

这篇工作面向 ICU 场景中的不规则时序分类，核心任务是基于高缺失、异步采样的 ICU 时间序列进行临床风险预测，重点实验是死亡风险分类。ICU 数据天然带有强烈的采样不规则性：生命体征、化验、治疗记录和病程事件不是按统一频率产生，而是由病情、报警阈值、医生决策和医院流程共同触发。因此，模型面对的不只是缺值矩阵，而是一个观测值、时间戳、变量选择和采样密度交织在一起的预测问题。

论文关注的核心难点是：LLM 是否真的给不规则 ICU 时序分类带来了实质收益，还是只是把已有时序表示接到大模型上而增加了计算成本。作者拆开考察两类关键组件：time-series encoder 和 multimodal alignment strategy，并将多种 LLM-based time series 方法与传统监督/自监督不规则时序模型比较。结果显示，显式建模 irregularity 的 encoder 比是否接入复杂 LLM alignment 更关键；相对 vanilla Transformer，irregularity-aware encoder 带来明显 AUPRC 提升，但 LLM-based 方法通常训练成本高出约一个数量级，性能却只达到与强不规则时序专用模型相当，且在 few-shot 场景下并不占优。

### 审稿人视角：价值与不足

最有价值的地方是它提供了一个“负结果也很有用”的系统性评测视角。近两年很多工作默认 LLM 能提升时序任务，但这篇论文把问题具体化到不规则 ICU 分类，并指出性能瓶颈主要在时序编码器是否理解时间间隔、缺失和异步观测，而不是在后端 LLM 推理能力。对审稿人而言，这种组件级拆解很重要：它避免把 LLM 当作黑箱增益来源，并提醒社区不要用更昂贵的多模态对齐掩盖前端 irregularity modeling 的不足。

不足也比较明显。它更像评测与诊断论文，而不是提出新的采样机制建模方法；任务主要集中在 ICU mortality prediction 和有限 benchmark 上，对 sepsis、AKI、多机构迁移、主动采样等更复杂场景覆盖不足。论文证明了 encoder 重要，但还没有进一步回答“encoder 应该如何区分病情驱动的 informative sampling 与医院流程驱动的 policy sampling”。此外，它比较的是当前主流 LLM-based pipeline，不能排除未来更强的结构化提示、工具调用或专门预训练的 clinical time-series LLM 改变结论。

### 对 Sampling-Policy Shift 的启发

这篇工作对 Sampling-Policy Shift 的横向启发是：策略偏移问题不能寄希望于 LLM 后端自动修复，前端不规则时序编码器仍是关键控制点。如果 encoder 已经把医院采样流程、测量密度或变量触发规则编码成类别证据，后续 LLM alignment 只会语义化地继承这些偏差。因此，我们的模型设计应优先在 encoder 层显式分解 state signal 与 policy signal，再决定哪些信息进入分类头、哪些信息进入策略诊断或不确定性校准。

纵向深化上，可以把该论文的评测思路扩展成 policy-shift benchmark：不仅比较 LLM-based 与 irregularity-aware encoder 的平均 AUPRC/AUROC，还要在跨医院、跨采样频率、跨变量测量协议和反事实采样增强下比较表示漂移。一个直接方案是构造三类前端：value-only encoder、timestamp/mask-aware encoder、policy-decomposed encoder，然后接相同 LLM 或相同分类头，观察性能差异是否来自稳定状态信息还是采样政策捷径。该论文的结论提醒我们：LLM 可以作为临床语义压缩或解释层，但 Sampling-Policy Shift 的核心仍要在观测过程建模、策略不变表征和反事实采样一致性上解决。
