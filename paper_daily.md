...
- 检索范围：近 3-7 个月内围绕 irregular sampled / asynchronous / irregular multivariate time series classification 的顶会论文，重点核对 ICML 2026 官方页面、OpenReview、arXiv 与官方实现页面。
- 已排除黑名单论文；同时排除偏规则时序分类、纯 forecasting/generation、workshop 条目或时间较旧的工作。本次保留全新工作 2 篇。

## 10. QuITE: Query-based Irregular Time-series Embedding

- 全称：Query-Based Irregular Time Series Embedding
- 会议：ICML 2026 Poster
- 作者：Junghoon Lim
- 链接：https://icml.cc/virtual/2026/poster/64962
...
- 关键词：irregular multivariate time series, query tokens, masked self-attention, plug-and-play embedding, classification/forecasting

### 场景、任务与核心难点

QuITE 面向不规则多变量时序建模，并明确包含分类下游任务。典型场景包括医疗监测、工业传感器、网络安全攻击流和环境观测：不同变量在不同时间被观测，时间间隔不均匀，且很多强大的 MTS backbone 默认输入已经是规则网格。若为适配这些 backbone 而先做插值、补齐或重采样，容易引入人工值并扭曲原始动态；若专门设计 irregular architecture，又会牺牲已有规则时序模型的复用能力。

论文的核心判断是：瓶颈不一定在 backbone，而在输入 embedding 层默认了均匀采样。QuITE 因此提出一个 backbone-agnostic 的输入嵌入模块，用一组可学习 query tokens 通过单层 masked self-attention 聚合原始不规则观测，直接生成可喂给标准 MTS 模型的固定维度潜表示。这样既避免了显式生成插值值，也不要求重写下游分类/预测模型结构。

### 审稿人视角：价值与不足

最有价值的思想是把“不规则时序如何进入规则 backbone”抽象为可替换的 embedding 问题。相比为每个任务设计完整的新模型，QuITE 把不规则采样处理压缩成轻量、可插拔的观察聚合器，让已有的 PatchTST、Mamba 类或 Transformer 类模型可以直接复用；query token 也比固定池化更灵活，因为它能学习哪些观测片段、变量和时间间隔对下游任务更重要。

不足在于，query attention 聚合并不自动区分“状态动力学导致的观测重要性”和“采样政策导致的观测重要性”。如果训练环境中某些类别被更密集监测，或者某些变量只在医生怀疑特定病情时出现，query tokens 很可能把采样密度、观测缺口和联测模式编码成判别特征。论文报告了多 backbone、多 benchmark 的平均增益，但主要仍是同分布评测；在跨医院、跨设备或主动采样策略变化下，learned queries 的语义是否稳定尚未被充分验证。此外，固定数量 query 生成的压缩表示可能掩盖稀有但关键的异步事件顺序。

### 对 Sampling-Policy Shift 的启发

QuITE 对我们的问题有直接的横向启发：可以把 query tokens 从“通用观测聚合器”改造成“状态 query + 策略 query”的解耦结构。状态 query 只负责抽取跨采样策略稳定的动态与类别证据，策略 query 则显式解释观测频率、缺失间隔、变量联测和采样触发规则；分类头主要连接状态 query，策略 query 用于偏移诊断和不确定性校准。

纵向深化上，QuITE 很适合作为 counterfactual sampling augmentation 的载体：对同一底层轨迹生成多种采样策略视图，要求状态 query 表征和 logits 保持一致，同时允许策略 query、attention pattern 和观测覆盖率发生变化。还可以把 query-token attention 当作 sampling-policy leakage 诊断工具：若换采样政策后关键 query 的注意力从状态事件转向观测密度或缺失空窗，就说明模型仍在依赖策略性捷径。

## 11. Generative Modeling of Irregular Time Series via SDE-Induced Continuous-Discrete Variational Inference

- 简称：SDEVI
- 会议：ICML 2026 Spotlight / Poster
- 作者：Zexin Yuan, Qinliang Su, Junxi Xiao
- 链接：https://icml.cc/virtual/2026/poster/64934
- 代码：https://github.com/SamoyedY/SDE-VI
- 关键词：irregular time series, sparse asynchronous observations, stochastic differential equation, continuous-discrete variational inference, classification/interpolation/regression

### 场景、任务与核心难点

SDEVI 面向稀疏、异步、不规则观测下的连续时间序列建模，实验覆盖 healthcare、physics、climate、IoT 等场景，并报告 interpolation、extrapolation、regression 和 classification 等下游任务。对于分类问题，这类数据的难点在于：真实系统通常有连续潜在动力学，但我们只能看到不均匀时间点上的离散观测；观测稀疏时，直接在离散点上训练分类器容易过拟合采样图样，而传统连续-离散 state-space model 又常依赖 path-based variational inference，计算昂贵且 posterior 假设受限。

论文提出 SDE-induced continuous-discrete variational inference：不再显式对整条连续路径做高成本变分推断，而是直接在离散观测的联合分布上做 inference，同时保证它与底层 SDE 连续过程一致。方法使用 linear time-varying SDE 诱导的 variational posterior 作为可扩展推断骨架，并进一步引入 nonlinear-SDE-induced variational inference 与 complex-domain 扩展，以覆盖更复杂的真实动力学。

### 审稿人视角：价值与不足

最有价值的技术思想是把“不规则观测上的离散 likelihood”和“连续时间随机动力学”严密连接起来。它避免了为每条样本反复求解或采样完整 latent path 的高成本，同时不退回到普通离散序列模型；对分类任务而言，这意味着模型可以通过连续潜过程学习更稳定的动态表征，而不是只依赖观测点排列。ICML Spotlight 身份也说明该工作在不规则时序生成建模和可扩展变分推断上具有较强方法贡献。

不足是，SDEVI 的目标仍主要是解释观测值联合分布；如果观测时间本身由采样政策、事件触发或机构流程决定，posterior uncertainty 和 latent state 仍可能混入 policy-induced information。SDE 一致性保证了连续动力学建模的形式合理性，但不保证学到的是跨采样策略不变的状态机制。另一个风险是 SDE 假设本身：某些 EHR、故障日志或网络安全事件具有突变、离散干预和非平稳采样触发，单纯连续扩散过程可能不足以表达观测机制。论文虽然覆盖分类任务，但尚未把 sampling-policy shift 作为显式评测维度。

### 对 Sampling-Policy Shift 的启发

SDEVI 给我们的纵向启发是：Sampling-Policy Shift 可以被放进连续-离散生成框架中统一处理。我们可以把真实状态建模为 latent SDE，把采样政策建模为条件观测过程或 marked point process，并要求分类器依赖 latent state dynamics，而不是直接依赖 observation-time process。这样，策略偏移不再只是 mask augmentation，而是生成模型中一个可识别、可反事实替换的组成部分。

横向应用上，可借鉴 SDEVI 的离散观测联合推断来设计 counterfactual policy likelihood：固定同一 latent trajectory，替换不同采样时间策略，检查 posterior state、classification logits 和 uncertainty 是否稳定。若换策略后 posterior 坐标大幅漂移，就说明模型仍把采样机制当作状态证据。进一步，可以把 variational posterior 分解为 state posterior 与 policy posterior，并在训练中加入跨策略 KL/Wasserstein 对齐，只对 state posterior 施加分类监督，对 policy posterior 保留为偏移检测与校准信号。
